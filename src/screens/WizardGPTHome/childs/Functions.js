import Helper from 'shared/helper';
  ...
  fn.ToInputJson = (DesignName = '', tables) => {
    const data = {
      model: {
        name: DesignName.split(/[\s-]/).join("_"),
        classes: tables.map(table => ({
          name: table.name,
          stereotype: table.stereotype || 'Entity',
          type: table.stereotype && table.stereotype.toLowerCase() !== 'entity' ? table.stereotype : 'Class',
      ...
      if (!Helper.IsJSONEmpty(deltaInfo)) {
        deltaClasses.forEach(cls => {
          if (cls.name === data.entity) {
            if (deltaInfo.type === 'Deleted') handleDeletedFields(cls, deltaInfo, data);
  ...
  const handleDeletedFields = (cls, deltaInfo, data) => {
  ...
  fn.validateModel = (model) => {
    ...
    model.classes.forEach((cls) => {
      ...
      const name =`${cls.name} ${cls.type === 'Class' ? 'Api' : cls.type}`;
      ...
      if (!allowedTypes.includes(cls.type.toLowerCase())) addError(validationErrors.errors, name, `Invalid class type: ${cls.type}`);

      if (cls.type.toLowerCase() === 'enum') {
        ...
        if (entityErrors.length) addError(validationErrors.errors, name, entityErrors,cls.type);
        return;
      }

      validateProperties(cls.properties, entityErrors, propertyNames);

      if (['entity', 'class'].includes(cls.type.toLowerCase())) {
